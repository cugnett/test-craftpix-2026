...
func _physics_process(delta: float) -> void:
	if playing:
		...
		song_position_in_beats = float(song_position / sec_per_beat) + beats_before_start
		_report_beat()
...
func play_with_beat_offset(num):
	...
	$StartTimer.start()
	print("timer start" + str(sec_per_beat))
